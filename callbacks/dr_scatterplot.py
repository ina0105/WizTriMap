...
from utils.utils import get_blank_image_base64
...
@app.callback(
    ...
    Output('empty-area-clicked', 'data', allow_duplicate=True),
...
def model_prog_trimap_scatterplot_clicked_display(click_data, scatterplot_fig, dataset, layer_number):
    ...
    with model_prog_image_lock:
        ...
        scatterplot_fig['data'][1]['x'] = [clicked_x]
        scatterplot_fig['data'][1]['y'] = [clicked_y]
        return reconstructed_image_object, original_image_object, scatterplot_fig, False
...
def model_prog_trimap_scatterplot_empty_area_clicked_display(click_data, dataset, layer_number, scatterplot_fig):
    ...
    with model_prog_image_lock:
        ...
        scatterplot_fig['data'][1]['x'] = [clicked_x]
        scatterplot_fig['data'][1]['y'] = [clicked_y]
        return reconstructed_image_object, original_image_object, scatterplot_fig, True
...
@app.callback(
    ...
    State('right-dropdown', 'value'),
    State('model-prog-plot', 'figure'),
    State('empty-area-clicked', 'data'),
    prevent_initial_call=True
)
def model_prog_trimap_radio_button(layer_number, dataset, scatterplot_fig, empty_area_clicked):
    global layer_number_change_lock
    with layer_number_change_lock:
        click_x = scatterplot_fig['data'][1]['x']
        click_y = scatterplot_fig['data'][1]['y']

        if len(click_x) == 1 and len(click_y) == 1 and not empty_area_clicked:
            marker_x, marker_y = dr_scatterplot.get_marker_points(dataset, layer_number, click_x, click_y,
                                                                  scatterplot_fig)

            new_plot = dr_scatterplot.create_scatterplot_figure_model_prog(dataset, layer_number,
                                                                           model_prog_scatterplot_width,
                                                                           model_prog_scatterplot_height,
                                                                           marker_x=[marker_x], marker_y=[marker_y])
            click_data = {'points': [{'x': marker_x, 'y': marker_y}]}
            reconstructed_image_object = recon_image.display_cnn_layer_emb_recon_image(click_data, dataset,
                                                                                       layer_number)
            original_image_object = original_image.display_cnn_layer_emb_original_image(click_data, dataset,
                                                                                        new_plot)
        else:
            ...
            reconstructed_image_object = get_blank_image_base64()
            original_image_object = get_blank_image_base64()

        return new_plot, reconstructed_image_object, original_image_object
